...
let init_state () =
  p "words";
  let words2 = List.map List.rev (List.map (List.map (Tag.int_of_tag)) !words) in
  p "nd_auto";
  let nd_auto = Finiteautomaton.make_setstar_naive words2 in
  ...
  let conf2_auto = Finiteautomaton.confuse_letters conf_auto (List.map int_of_tag [ADJ; VER Pper; VER Ppre]) in
  p "deteminize";
  let det_auto = Finiteautomaton.determinize conf2_auto in
